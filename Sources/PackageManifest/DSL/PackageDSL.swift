extension Package {
    public func dependencies(@DependenciesBuilder _ builder: () -> [AnyDependency]) -> Package {
        let package = self
        package.dependencies = builder().map{ $0.underlying }
        return package
    }

    public func modules(@ModulesBuilder _ builder: () -> [AnyModule]) -> Package {
        let package = self
        package.modules = builder().map { $0.underlying }
        return package
    }

    public func minimumDeploymentTarget(@DeploymentTargetsBuilder _ builder: () -> [DeploymentTarget]) -> Package {
        let package = self
